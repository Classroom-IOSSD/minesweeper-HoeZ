...
int uncover_blank_cell(int row, int column) {
	...
	columnumns[0] = column + 1;

	rows[1] = row;

	columnumns[1] = column + 1;

	rows[2] = row + 1;

	columnumns[2] = column + 1;
	...
	columnumns[5] = column - 1;

	rows[6] = row;

	columnumns[6] = column - 1;

	rows[7] = row + 1;

	columnumns[7] = column - 1;



	for (i = 0; i < 8; i++) {

		value = table_array[rows[i]][columnumns[i]];

		bool isValid = (rows[i] >= 0 && rows[i] < MAX) && (columnumns[i] >= 0 && columnumns[i] < MAX);

		if (isValid) {	// to prevent negative index and out of bounds

			if (value > 0 && value <= 8)

				table_array[rows[i]][columnumns[i]] += 10;										// it is a cell with 1-8 number so we need to uncover

			else if (value == 0)

				uncover_blank_cell(rows[i], columnumns[i]);
...
int main(int argc, char *argv[]) {
	...
	int i, j, r, c, value, rows[8], columnumns[8];
	...
	while (1) {
		...
		for (i = 0; i < number_Mines; i++) {
			...
			if (table_array[r][c] != 99) {
				...
				for (j = 0; j < 8; j++) {
					...
					value = table_array[rows[j]][columnumns[j]];
					...
					if (isValid) {	// to prevent negative index and out of bounds
						...
						if (value != 99)																// to prevent remove mines
							...
							table_array[rows[j]][columnumns[j]] += 1;									// sums 1 to each adjacent cell
		...
		while (number_Mines != 0) {			// when numberMines becomes 0 you will win the game
